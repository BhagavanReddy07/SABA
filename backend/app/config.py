...
class Settings(BaseSettings):
    # ====== AI Keys ======
    GEMINI_API_KEYS: Optional[str] = Field("dummy-key-for-testing", env="GEMINI_API_KEYS")  # Comma-separated
    ...
    REDIS_URL_CELERY: str = Field("redis://localhost:6379/0", env="REDIS_URL_CELERY")  # For Celery
    REDIS_URL_CHAT: str = Field("redis://localhost:6379/1", env="REDIS_URL_CHAT")      # For chat history
    REDIS_CHAT_HISTORY_KEY: str = Field("chat_history", env="REDIS_CHAT_HISTORY_KEY")  # last 10 messages

    # ====== Neo4j ======
    NEO4J_URI: str = Field("bolt://localhost:7687", env="NEO4J_URI")
    NEO4J_USER: str = Field("neo4j", env="NEO4J_USER")
    NEO4J_PASSWORD: Optional[str] = Field("password", env="NEO4J_PASSWORD")
